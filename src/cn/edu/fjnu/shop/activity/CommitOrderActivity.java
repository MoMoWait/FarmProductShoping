	...
	public void initData(){
		...
		aQuery.id(R.id.img_commit_order).clicked(new View.OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
				if(account<all){
					OPUtils.showToast("资金不足,请充值",Toast.LENGTH_SHORT);
					return;
				}else{
	...
	public void responsePersonalInfo(String url,JSONObject data,AjaxStatus status){
		...
		if(data!=null){
			try {
				...
				OPUtils.showToast("个人信息获取失败", Toast.LENGTH_SHORT);
		...
		if(data!=null){
			try {
				...
				if("success".equals(orderState)){
					closeLoadingDialog();
					OPUtils.showToast("订单提交成功", Toast.LENGTH_SHORT);
				//	closeLoadingDialog();
					setResult(RESULT_OK);
					CommitOrderActivity.this.finish();
					overridePendingTransition(R.anim.activity_exit_right, R.anim.activity_exit_left);
					...
					closeLoadingDialog();
					OPUtils.showToast("订单提交失败", Toast.LENGTH_SHORT);
				}
