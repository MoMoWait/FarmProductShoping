...
public class ProductDetailActivity extends Activity implements OnClickListener{
	...
	public void initView(){
		...
		photoImageView.setLayoutParams(
				new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, (int)(CommonValues.screenPixWidth*0.75f)));
		
		
		contentScrollView = (ScrollView)findViewById(R.id.layout_content);
		contentScrollView.setLayoutParams(
				new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, CommonValues.screenPixHeight- SizeUtils.dp2px(120)));
		...
		aQuery.id(R.id.text_add_to_cart).clicked(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(UserInfo.userID==-1){
					...
					OPUtils.startActivity(ProductDetailActivity.this, LoginActivity.class);
						...
						if(object!=null){
							try {
								...
								if("addSuccess".equals(state)){
									OPUtils.showToast("关注成功", Toast.LENGTH_SHORT);
									aQuery.id(R.id.img_my_watch).image(R.drawable.watch_red);
									//accessDialog.dismiss();
								}else if("deleteSuccess".equals(state)){
									OPUtils.showToast("取消关注", Toast.LENGTH_SHORT);
									aQuery.id(R.id.img_my_watch).image(R.drawable.watch);
								}else{
									OPUtils.showToast("服务器异常", Toast.LENGTH_SHORT);
								}
									
								
							} catch (Exception e) {
								// TODO: handle exception
								OPUtils.showToast("操作失败", Toast.LENGTH_SHORT);
								e.printStackTrace();
							}
						}
					}
		...
		if(data!=null){
			try {
				...
				if("success".equals(state)){
					...
					OPUtils.showToast("添加成功",250);
					
					//ProductDetailActivity.this.finish();
				}else{
					DialogUtils.closeLoadingDialog();
					OPUtils.showToast("添加失败",Toast.LENGTH_SHORT);
				}
			} catch (Exception e) {
				// TODO: handle exception
				DialogUtils.closeLoadingDialog();
